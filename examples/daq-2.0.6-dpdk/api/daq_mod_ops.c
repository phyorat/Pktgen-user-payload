...
DAQ_LINKAGE int daq_rsp_pc_filter(const DAQ_Module_t *module,
        void *handle,
        const void *pc_data,
        int datalen,
        DAQ_Set_PktCnt_Filter filter_cb)
{
    if (!module)
        return DAQ_SUCCESS;
    ...
    return module->rsp_pc_filter(handle, pc_data, datalen, filter_cb);
}

DAQ_LINKAGE int daq_req_pc_filter(const DAQ_Module_t *module,
        void *handle,
        void *filter_ptr,
        daq_pc_filter_req_type req_type)
{
    if (!module)
        return DAQ_ERROR_NOMOD;
    ...
    return module->req_pc_filter(handle, filter_ptr, req_type);
}
