...
test_pktgen_packet_capture_init(void)
{
	...
	cmp_ok(_c.mz->len,
	       "==",
	       sizeof(_mz_pool),
	       "    ... and a reasonably sized memory pool");
	cmp_ok(_c.mz->socket_id,
	       "==",
	       socket_id,
	       "    ... on the requested socket");
	ok(_c.mz->flags & RTE_MEMZONE_1GB,
	   "    ... and preferring 1GB hugepages");
	ok(_c.mz->flags & RTE_MEMZONE_SIZE_HINT_ONLY,
	   "    ... but allowing fallback to available page size");
}
...
void
test_pktgen_packet_capture_bulk(void)
{
	int nb_dump = 0;
